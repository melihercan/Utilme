...
[![NuGet](https://img.shields.io/nuget/v/Utilme.SdpTransform.svg)](https://www.nuget.org/packages/Utilme.SdpTransform)
...
Individual SDP fields or the entire SDP structure can be converted to text, object, or JSON
representations.
...
📖 **[Overview and known issues in the wiki](https://github.com/melihercan/Utilme/wiki/SdpTransform)**
— full documentation is pending a refactor, so the example below remains the primary usage guide.
...
When you run the above program (JSON data is displayed as a screenshot):
```
SDP OBJECT TO TEXT:
v=0
o=mediasoup-client 10000 2 IN IP4 0.0.0.0
...
Deserializing that JSON back into an `Sdp` and calling `ToText()` again prints the same 55 lines,
byte for byte. That round-trip fidelity — text → object → JSON → object → text — is the point of the
demo, and the check to run after changing the library.
